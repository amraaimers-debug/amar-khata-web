...
export default function HomeClient({ initialPosts, recentComments, avatarUrl }) {
  ...
  useEffect(() => { setTypeFilter(searchParams.get("type") || "সব"); }, [searchParams]);
  useEffect(() => {
  ...
  async function toggleLike(id) {
    ...
    if (isLiked) {
      ...
      setLikedIds((p) => { const next = [...p, id]; localStorage.setItem("amarkhata_liked", JSON.stringify(next)); return next; });
      setPosts((p) => p.map((x) => (x.id === id ? { ...x, likes_count: x.likes_count + 1 } : x)));
    }
  }
  ...
  useEffect(() => { setVisibleCount(6); }, [view, typeFilter, tag, query]);
    ...
    <div>
      ...
      {allTags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-6">
          ...
          {allTags.map((t) => (
            <button key={t} onClick={() => setTag(tag === t ? null : t)} className="text-xs px-3 py-1 rounded-full border"
              style={{ borderColor: "var(--line)", background: tag === t ? "var(--gold)" : "var(--surface)", color: tag === t ? "#231A05" : "var(--ink)" }}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-8 items-start flex-col md:flex-row">
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <h2 className="font-display text-xl">{view === "saved" ? "সংরক্ষিত লেখা" : "সাম্প্রতিক লেখা"}</h2>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex flex-wrap rounded-full overflow-hidden border" style={{ borderColor: "var(--line)" }}>
                {["সব", ...ALL_TYPES].map((f) => (
                  <button key={f} onClick={() => setTypeFilter(f)} className="text-xs font-semibold px-3 py-1.5"
                    style={{ background: typeFilter === f ? "var(--maroon)" : "transparent", color: typeFilter === f ? "#fff" : "var(--muted)" }}>
                    {f}
                  </button>
                ))}
              </div>
              <button onClick={() => setView(view === "saved" ? "home" : "saved")} className="text-xs font-semibold px-3 py-1.5 rounded-full border" style={{ borderColor: "var(--line)", color: "var(--muted)" }}>
          ...
          {hasMore && (
            <div className="flex justify-center mt-6">
              <button
                ...
                className="px-5 py-2 rounded-full border font-semibold text-sm"
                style={{ borderColor: "var(--line)", color: "var(--ink)" }}
              >
                আরও লেখা দেখুন ({visible.length - visibleCount}টি বাকি)
              </button>
            </div>
          )}
        </div>
        ...
        <aside className="w-full md:w-[270px] flex-none flex flex-col gap-5">
          <div className="rounded-lg p-5" style={{ border: "1px solid var(--line)", background: "var(--surface)" }}>
            ...
            {popular.map((p, i) => (
              <a key={p.id} href={`/post/${p.id}`} className="flex items-baseline gap-2 py-2 border-b border-dotted last:border-none" style={{ borderColor: "var(--line)" }}>
                ...
                <span className="text-sm flex-1 hover:text-[var(--maroon)]" style={{ color: "var(--ink)" }}>{p.title}</span>
          ...
          <div className="rounded-lg p-5" style={{ border: "1px solid var(--line)", background: "var(--surface)" }}>
            ...
            {(!recentComments || recentComments.length === 0) && (<p className="text-xs" style={{ color: "var(--muted)" }}>এখনো কোনো মন্তব্য নেই।</p>)}
